...
class ItemRecyclerHeaderHolder private constructor(val binding: ItemHeaderDateBinding) :
    ...
    fun bind(item: RecycleViewEntity, listener: HolderItemClickListener) {
        binding.headerDate.text = SimpleDateFormat("EEEE , dd MMMM yyyy").format(item.date)
    ...
    companion object {
        fun from(parent: ViewGroup): ItemRecyclerHeaderHolder {
            val binding =
                ItemHeaderDateBinding.inflate(LayoutInflater.from(parent.context), parent, false)
